...
BAZALT_BUILD_ZENOH_CPP_VERSION=${BAZALT_BUILD_ZENOH_VERSION:-latest}
...
clone(){
  if [ "${BAZALT_BUILD_ZENOH_CPP_VERSION}" = "latest" ]; then
      BAZALT_BUILD_ZENOH_CPP_VERSION="$(curl -s https://api.github.com/repos/eclipse-zenoh/zenoh-cpp/releases/latest | jq -r .tag_name)";
  fi
  echo "Cloning zenoh-cpp version ${BAZALT_BUILD_ZENOH_CPP_VERSION}..."
  git clone "${BAZALT_BUILD_ZENOH_CPP_SOURCE_REPOSITORY_URL}" "${BAZALT_BUILD_ZENOH_CPP_SOURCE_DIR}" \
    --depth 1 \
    --branch "${BAZALT_BUILD_ZENOH_CPP_VERSION}"
}
